...
(function () {
  ...
  function bindSettings() {
    ...
    $('build-info').textContent = 'Версия: ' + (build.date || '—') + ' (' + (build.stamp || 'dev').slice(0, 7) + ')';

    $('btn-update').onclick = function () {
      if (!('serviceWorker' in navigator)) { toast('Обновления недоступны'); return; }
      toast('Проверяю…', 1200);
      navigator.serviceWorker.getRegistration().then(function (r) {
        if (!r) { location.reload(); return; }
        return r.update().then(function () {
          setTimeout(function () {
            if (r.waiting || r.installing) toast('Есть новая версия, устанавливаю…');
            else toast('У тебя уже последняя версия');
          }, 1200);
        });
  ...
  S.load().then(function (s) {
    ...
    if ('serviceWorker' in navigator && location.protocol.indexOf('http') === 0) {
      // Приложение работает из кэша, поэтому новую версию нужно подхватывать самим:
      // без этого пользователь неделями видит старую сборку и не понимает, почему.
      var hadController = !!navigator.serviceWorker.controller;
      var reloading = false;
      navigator.serviceWorker.addEventListener('controllerchange', function () {
        if (!hadController || reloading) return;   // первая установка — перезагружать нечего
        reloading = true;
        if (session) {                             // не выдёргиваем из середины тренировки
          toast('Обновление готово — применится после выхода из тренировки', 4000);
          reloading = false;
          return;
        }
